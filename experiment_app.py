...
3. **风险推演：** 依据埃及现行环保法规，保护区内项目绝对不适用 EG-GEA-2024 法案的免税红利。财务模型中 12.4% 的 IRR 存在致命的高估错误。
         """, 
         "is_faulty": True}, # 陷阱：沙姆沙伊赫根本不在尼罗河三角洲，AI在胡说八道
...
3. **风险推演：** 采用该 EPC 方案将直接触发 MDB 的反洗钱及反制裁（AML/Sanctions）违约条款。不仅项目会立即遭到抽贷，贵司也将面临次级制裁的高危传染风险。
         """, 
         "is_faulty": True}, # 陷阱：AI 捏造了一个极其严重的制裁名单幻觉，看风控高管会不会被吓住而放弃。
...
3. **商业权衡：** 延期造成的沉没成本（>500万美金）已严重侵蚀预期利润。建议立即启动亚洲备选供应商的联合技术攻关与 RFNBO 认证辅导，实现供应链去风险化（De-risking）。
         """, 
         "is_faulty": False}
    ]
}
    ...
    if idx < len(active_projects):
        ...
        if ready:
            ...
            if f"waited_{idx}" not in st.session_state:
                time.sleep(2.0) # 稍微加长一点等待时间，显得计算很复杂
                st.session_state[f"waited_{idx}"] = True
            
            # 【升级】使用 markdown 渲染思维链
            ...
            with st.container(border=True):
                ...
                # 【新增】强制问责输入框（Cognitive Forcing）
                rationale = st.text_input("📝 请简述支撑您做出此决策的1-2个核心依据（必填项）：", key=f"rationale_{idx}", placeholder="例如：法案合规风险过大 / AI忽略了某项参数...")
                
                elapsed_since_reveal = time.time() - st.session_state[f"ai_reveal_time_{idx}"]
                
                # 只有当选项选了，且理由框写了至少3个字，才能提交
                ...
                if st.button(btn_label, type="primary", disabled=btn_disabled, key=f"btn_{idx}"):
                    ...
                    row = {
                        ...
                        "confidence": conf,
                        "rationale_text": rationale, # 新增记录受试者的思考理由
